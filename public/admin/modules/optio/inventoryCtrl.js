app.controller('InventoryCtrl', ['$scope', 'categoryService', 'mediaService', 'optioService', '$cookieStore', '$filter', function($scope, categoryService, mediaService, optioService, $cookieStore, $filter) {
    ...
    $scope.search = function() {
        ...
        angular.forEach($scope.allOptio, function (optio) {
            if ((optio.oLMedia != null &&optio.oLMedia.mName.toLowerCase().search($scope.searchTag.toLowerCase()) > -1) || (optio.oRMedia != null && optio.oRMedia.mName.toLowerCase().search($scope.searchTag.toLowerCase()) > -1) || (optio.oCategory!= null && optio.oCategory.cName.toLowerCase().search($scope.searchTag.toLowerCase()) > -1)) $scope.searchOptio.push(optio);
        });
    }
