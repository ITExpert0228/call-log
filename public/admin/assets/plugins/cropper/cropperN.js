...
(function (factory) {
  ...
  Cropper.prototype = {
    ...
    scale: function (scaleX, scaleY) {
      ...
      if (!this.built || this.disabled || !this.defaults.rotatable || !this.support.canvas) {
        return;
      }

      // this.rotated = TRUE;
      scaleX = image.scaleX * scaleX;
      scaleY = image.scaleY * scaleY;

       // replace with "true" to prevent to override the original image
      this.replace(this.getScaleDataURL(scaleX, scaleY), true);
    },

    getScaleDataURL: function (scaleX, scaleY) {
      var canvas = $("<canvas>")[0],
          context = canvas.getContext("2d"),
          originalImage = this.originalImage,
      ...
      context.drawImage(this.$original[0], 0, 0);
      context.restore();
